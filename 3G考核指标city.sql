...
(CASE
WHEN tab_six.city = 'Baoji' THEN '宝鸡'
WHEN tab_six.city = 'Xianyang' THEN '咸阳'
WHEN tab_six.city = 'Tongchuan' THEN '铜川'
WHEN tab_six.city = 'Hanzhong' THEN '汉中'
WHEN tab_six.city = 'Yulin' THEN '榆林'
WHEN tab_six.city = 'Yanan' THEN '延安'
WHEN tab_six.city = 'Shangluo' THEN '商洛'
WHEN tab_six.city = 'Other' THEN '其他'
ELSE NULL END) as "地市",
...
sum(tab.RRC建立成功率_X) as RRC建立成功率_X,
sum(tab.RRC建立成功率_Y) as RRC建立成功率_Y,
...
sum(tab.语音rab建立成功率_X) as 语音rab建立成功率_X,
sum(tab.语音rab建立成功率_Y) as 语音rab建立成功率_Y,
...
sum(tab.语音业务掉话率_X) as 语音业务掉话率_X,
sum(tab.语音业务掉话率_Y) as 语音业务掉话率_Y,


/* servlev.分组业务掉线率 AS "分组域业务掉话率(%)", */
round(decode(sum(tab.分组业务掉线率_Y),0,null,
sum(tab.分组业务掉线率_X)/sum(tab.分组业务掉线率_Y))*100,2) AS "分组域业务掉话率(%)",
sum(tab.分组业务掉线率_X) as 分组业务掉线率_X,
sum(tab.分组业务掉线率_Y) as 分组业务掉线率_Y,
...
sum(tab.软切换比例_X)/sum(tab.软切换比例_Y))*100,2) AS "软切换比例(%)",
...
sum(tab.同频硬切换成功率_X) as 同频硬切换成功率_X,
sum(tab.同频硬切换成功率_Y) as 同频硬切换成功率_Y,
...
sum(tab.RNC异频硬切换成功率_X) as RNC异频硬切换成功率_X,
sum(tab.RNC异频硬切换成功率_Y) as RNC异频硬切换成功率_Y,


/* cellres.小区上行平均rtwp AS "RTWP平均值(dBm)", */
round(avg(tab.小区上行平均rtwp),2) AS "RTWP平均值(dBm)",
