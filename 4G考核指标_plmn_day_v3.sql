SELECT
    tab1.DDATE AS DDATE,
  --  to_char(tab1.DDATE,'yyyy-mm-dd') AS DDATE,
    ...
    FROM 
        ...
        FROM 
            ...
            INNER JOIN C_LTE_CUSTOM c ON c.lncel_objid = lcelav.lncel_id and c.city is not null
                        and  c.lnbtsid   in (773522,773529,773568,773714,773716,773800,773807,773815,773821,773823,773843,773849,773883,773892,773939,773957,774123,774142,778263,778264,778289,778521,778615,782393,782519,782778,772460,772463,772654,772698,773005,773240,773242,773426,773486,773501,773513,773567,773581,773645,773681,773700,773715,773718,773845,773861,773863,773882,773895,773896,774060,774129,776284,776285,776289,776453,778241,778242,778248,778252,778255,778258,778275,778285,778287,778592,778887,778923,778967,778993,779043,779425,779604,782473,782477,782514,782626,782907,782908,783119,783141,783232,783378,783399)
        WHERE 
...
WHERE
        tab1.DDATE = tab2.DDATE AND tab1.lncel_enb_id = tab2.lncel_enb_id and tab1.lncel_lcr_id = tab2.lncel_lcr_id
    AND tab1.DDATE = tab3.DDATE AND tab1.lncel_enb_id = tab3.lncel_enb_id and tab1.lncel_lcr_id = tab3.lncel_lcr_id
    AND tab1.DDATE = tab4.DDATE AND tab1.lncel_enb_id = tab4.lncel_enb_id and tab1.lncel_lcr_id = tab4.lncel_lcr_id
    AND tab1.DDATE = tab5.DDATE AND tab1.lncel_enb_id = tab5.lncel_enb_id and tab1.lncel_lcr_id = tab5.lncel_lcr_id
